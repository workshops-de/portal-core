...
module Jekyll
  ...
  # Liquid filter for handling image URLs
  module ImageUrlFilter
    def resolve_image_url(header_image, post_slug)
      return '' unless header_image

      if header_image.start_with?('http://', 'https://')
        # External URL - return as is
        header_image
      else
        # Local image - combine with post slug
        "/artikel/#{post_slug}/#{header_image}"
      end
    end
  end
end
